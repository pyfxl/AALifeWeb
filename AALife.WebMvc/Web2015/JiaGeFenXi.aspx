<%@ Page Title="消费价格分析" Language="C#" MasterPageFile="UserControl/MasterPage.master" AutoEventWireup="true" Inherits="JiaGeFenXi" Codebehind="JiaGeFenXi.aspx.cs" %>
...
<script type="text/javascript">
    ...
    $(function () {
        $("#datechoose").datepicker({
            ...
            onSelect: function (date, format) {
                location.href = "JiaGeFenXi.aspx?date=" + date;
            },
            onChangeMonthYear: function(year, month, format) {
                location.href = "JiaGeFenXi.aspx?date=" + year + "-" + fullmonth(month) + "-" + "<%=Convert.ToDateTime(Session["TodayDate"]).ToString("dd") %>";
            }
        });
        ...
        function longdata(){
            if(($(this).scrollTop() + $(this).height()) == $(this).get(0).scrollHeight) {
                ...
                $.ajax({
                    type: "post",
                    url: "AutoJiaGeFXJson.aspx?page=" + page,
                    dataType: "json",
                    cache: false,
                    beforeSend: function () {
                        if(!load) loading();
        ...
        function log(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                $(".maindiv table").eq(0).append("<tr><td>" + data[i].ItemTypeName + "</td><td>" + data[i].ItemName + "</td><td>" + data[i].ItemBuyDate + "</td><td class='cellprice'>￥" + data[i].ItemPrice + "</td><td><a href='JiaGeFenXiMingXi.aspx?itemName=" + data[i].ItemName + "&itemType=" + data[i].ItemType + "'>查看详情</a></td></tr>");
            }
        }        
        function loading() {
            load = true;
    ...
    function chart_click(index) {
        location.href = "JiaGeFenXiMingXi.aspx?" + arrChart[index];
    }
</script>    
</asp:Content>
    ...
    <div class="maindiv">
        <div class="chartdiv">
            ...
            <script type="text/javascript">
                ...
                { "data-file": "JiaGeFXChartJson.aspx" }, { "wmode": "transparent" }
                );
            </script>
            <div id="my_chart"></div>
        ...
        <table cellspacing="0" border="1" style="width:100%;" class="tablelist">
            ...
            <tr>
                ...
                <td><a href="JiaGeFenXiMingXi.aspx?itemName=<%# HttpUtility.UrlEncode(Eval("ItemName").ToString()) %>&itemType=<%# Eval("ItemType") %>">查看详细</a></td>
            </tr>
            </ItemTemplate>
            <FooterTemplate>
            <tr>

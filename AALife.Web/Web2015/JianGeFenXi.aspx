<%@ Page Title="消费间隔分析" Language="C#" MasterPageFile="UserControl/MasterPage.master" AutoEventWireup="true" CodeFile="JianGeFenXi.aspx.cs" Inherits="JianGeFenXi_2015" %>
    ...
    $(function () {
        $("#datechoose").datepicker({
            ...
            onChangeMonthYear: function(year, month, format) {
                location.href = "JianGeFenXi.aspx?date=" + year + "-" + fullmonth(month) + "-" + "<%=Convert.ToDateTime(Session["TodayDate"]).ToString("dd") %>";
            }
        });
        ...
        function log(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                $(".maindiv table").eq(0).append("<tr><td>" + data[i].ItemTypeName + "</td><td>" + data[i].ItemName + "</td><td><a href='ItemList.aspx?date=" + data[i].ItemBuyDate + "'>" + data[i].ItemBuyDate + "</a></td><td><a href='ItemList.aspx?date=" + data[i].ItemBuyDateMax + "'>" + data[i].ItemBuyDateMax + "</a></td><td>" + data[i].NotBuy + " 天</td></tr>");
            }
        }        
        function loading() {
            load = true;
